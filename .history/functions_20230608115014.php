...
function posts_link_next_class($format){
	$format = str_replace('href=', 'class="next text-black rounded-full w-fit h-[60px] bg-slate-200" href=', $format);
	return $format;
}
add_filter('next_post_link', 'posts_link_next_class');

function posts_link_prev_class($format) {
	$format = str_replace('href=', 'class="prev text-black rounded-full w-fit h-[60px] bg-slate-200" href=', $format);
	return $format;
}
