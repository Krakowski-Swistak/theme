...
if ( $product ) {
	if ( tutor_utils()->is_course_added_to_cart( $product_id, true ) ) {
		...
		<form action="<?php echo esc_url( apply_filters( 'tutor_course_add_to_cart_form_action', get_permalink( get_the_ID() ) ) ); ?>" method="post" enctype="multipart/form-data">
			<button type="submit" name="add-to-cart" value="<?php echo esc_attr( $product->get_id() ); ?>"  class="w-full flex justify-center items-end mt-6 p-4 border border-solid border-[#132787] bg-[#132787] group hover:bg-white rounded-sm shadow-[12px_12px_24px_0px] shadow-[rgba(51_79_211/0.16)] transition duration-200 <?php echo esc_attr( $required_loggedin_class ); ?>">
				<span class="text-white group-hover:text-[#22272F] uppercase font-medium text-sm transition duration-200"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></span>
